// 平台（完美世界等）下载的 demo 是标准 zip，内含 .dem。
// 零依赖解包：EOCD + 中央目录 + 本地头，校验 PBDEMS2 魔数，限制解压后大小。
import fs from "fs";
...
function readCentralDirectory(zip: Buffer, eocdOffset: number): ZipEntry[] {
  ...
  if (cdOffset + 46 > zip.length) throw new ZipDemError(400, "zip 中央目录损坏");
  ...
  for (let i = 0; i < entryCount; i += 1) {
    ...
    entries.push({
      ...
      name: zip.toString("utf8", offset + 46, offset + 46 + nameLen),
    });
...
function pickDemoEntry(entries: ZipEntry[]): ZipEntry {
  const files = entries.filter((e) => !e.name.endsWith("/"));
  if (!files.length) throw new ZipDemError(400, "zip 中没有文件");
  const named = files.find((e) => e.name.toLowerCase().endsWith(".dem"));
...
function inflateEntry(zip: Buffer, entry: ZipEntry): Buffer {
  ...
  if (dataEnd > zip.length) throw new ZipDemError(400, "zip 数据不完整，文件可能下载损坏");
...
export function extractDemFromZip(zipPath: string): {
  buffer: Buffer;
  entryName: string;
} {
  let zip: Buffer;
  ...
  if (zip.length < 22) throw new ZipDemError(400, "不是有效的 zip 文件");
  const eocdOffset = findEocd(zip);
  if (eocdOffset < 0) throw new ZipDemError(400, "不是有效的 zip 文件（找不到目录结尾记录）");
  const entry = pickDemoEntry(readCentralDirectory(zip, eocdOffset));
  const dem = inflateEntry(zip, entry);
  if (dem.length < DEMO_MAGIC.length || !dem.subarray(0, DEMO_MAGIC.length).equals(DEMO_MAGIC)) {
    throw new ZipDemError(400, `zip 中的 ${entry.name} 不是有效 CS2 demo（缺少 PBDEMS2 文件头）`);
  }
